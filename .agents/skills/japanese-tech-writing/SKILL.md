...
description: "日本語の技術文書・書籍原稿の文章規範を適用して執筆・推敲するスキル。整形（一文一行、脚注、太字と「」の使い分け）、パラグラフライティング、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章、記事、解説文、README、設計文書、ブログ記事などの文章を書く、推敲する、リライトする、校正する場合に必ず使用する。「文章を直して」「LLM っぽさを消して」「読みやすくして」のような依頼や、文体・構成のレビューにも使用する。対象は日本語の地の文であり、コードコメントやコミットメッセージには適用しない。"
user-invocable: true
---
...
規範は「よい文章の一般論」ではなく、技術文書の論証を成立させるための具体的な規則である。
執筆と推敲のどちらでも、規範の本文を全部読んでから作業する。
...
1. 後述の規範をすべて読む
2. 規範に従って書く。とくに「整形」「段落と論証の構成」「視点と語り」は書きながら適用する
3. 書き上げたら、下の点検パスを実行する

### 推敲・リライトするとき

1. 対象を特定する。引数 `$ARGUMENTS` が指定されていればそのファイル・文章を対象とし、なければ会話中の直近の原稿を対象とする
2. 下の点検パスを実行する
3. 依頼が「指摘」なら、規範名と該当箇所を対応づけて列挙する。依頼が「修正」なら、修正した本文を提示する

### 点検パス

書き上げた文章を、次の順で規範に照らして点検する。
構成の修正が文単位の修正を無効にするため、大きい単位から小さい単位へ進む。
...
4. LLM っぽい表現の禁止
5. 演出の抑制
6. 視点と語り、読み手の負荷の管理、見出しの付け方、読者への誠実さ
7. 整形

### 適用上の注意

規範は機械的な置換規則ではない。
多くの規範には「〜の場合は残す」「〜に限り使ってよい」という但し書きがあり、それを無視した一括置換は文章を壊す。
とくに、推量や不確実性を表す表現を機械的に断定へ変えることは、「論証の厳密さ」が明示的に禁じている。
推敲では、著者の主張の内容を変えない。変えるのは主張の運び方と表現である。
...
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを省く。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名である。
...
- 本文中の太字強調を多用しない。誤読を防ぐ否定や節の帰結など、論理の要所に限り、一節に一、二箇所まで使ってよい（導入部でも可）。ただし、初出の定義語の太字はこの上限に数えない。それ以外は文の順序と構造で際立たせる。
...
規範本文は k16shikano 氏の gist ([SKILL.md](https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d)) に基づく。
原典のライセンスは Unlicense (作者が同 gist のコメント欄で表明)。
本スキルでは description の調整、適用手順の追加、軽微な字句修正を加えている。
